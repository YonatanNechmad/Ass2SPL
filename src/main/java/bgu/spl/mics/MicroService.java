...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class, Callback> callBackMaps;
    ...
    public final void run() {
        ...
        subscribeBroadcast(TerminationBroadcast.class, (TerminationBroadcast TB) -> {
            Diary.getInstance().setTerminationTime(this);
            terminate();
        });
        initialize();
        Main.counter.countDown();
        Message m = null;
        while (!terminated) {
            try {
                m = MBus.awaitMessage(this);
            } catch (InterruptedException exception) {
            }
            if (m != null) {
                Callback c = callBackMaps.get(m.getClass());
                if (c != null) {
                    c.call(m);
                }
            }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        callBackMaps.put(type, callback);
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        Future future = MBus.sendEvent(e);
        return future;
    }
